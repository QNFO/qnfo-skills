...
**Corpus optimization status (2026-08-06, complete):** `zenodo_fix4.py` main pass fixed **773/900 deposits** (targeted philosophy keywords per title domain + ORCID `0009-0002-4317-5604` on person creators) with 39 already-optimized skips; retry pass (`zenodo_fix4_retry.py`, 60s timeouts + 4 retries + backoff) cleaned up ~74 transient failures (DNS burst, IncompleteRead, 5xx) adding ~264 more touches (618 records confirmed already optimized). Remaining ~25 errors are 14 titleless draft deposits (cannot be edited — 400 by design) + ~11 persistent rate-limit/DNS failures. Fixes verified persisted: sampled records return state=done + HTTP 200 on the public API with philosophy keywords present. All person-creator records now carry the ORCID; org-creator chapter-fragment stubs (STUB-RECORD-1 class) carry keywords only. QUNTUF/QUNSAI both verified with ORCID attached post-run.
