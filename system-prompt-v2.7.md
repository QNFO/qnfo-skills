> **v3.93 UPDATE (2026-08-29, kaizen — CMD SKILLS UPDATE: DPRF-cycle infra gates — D1-WRITE-ASCII-1 + EXEC-AUTOBG-SESSION-ERROR-1 + GIT-REBASE-AFTER-COMMIT-1 + FRAMEWORK-DOGFOOD-1 + REDTEAM-CHILD-CROSS-CHECK-1; mirrors kaizen v2.113 + cloudflare v3.68 + qnfo-core v1.38 + research v2.145 + execution-mandate v2.12):**
> Red-team: direct parent-agent skills audit (session this — QNFO.RES.032 DPRF P0/P0.1 + red-team gate cycle). HARD: 4. Changes:
> (1) [HARD] **D1-WRITE-ASCII-1** — D1 TEXT writes via curl JSON payloads from Git Bash MUST be ASCII-only (or \uXXXX-escaped): non-ASCII bytes (em-dash, §) are stored as raw CP1252 bytes and corrupt the field (rendered U+FFFD); "D:\path" inside a JSON string parses as \r (carriage return) — escape as D:\\. Fix = rewrite the whole field ASCII-only + re-query verify. Canonical: 2026-08-29 RES.032 registry description (protocol §8 corrupted → Sec.8 full rewrite, re-query verified).
> (2) [HARD] **EXEC-AUTOBG-SESSION-ERROR-1** — exec "Session X is not running" errors are a reporting glitch: the sessions complete normally (process list shows done / exitCode 0; output lands in the session log). Handle: one retry, then process log read-back — never assume the command did not run. Canonical: 2026-08-29 DPRF cycle (multiple false errors; all outputs recovered via logs).
> (3) [HARD] **GIT-REBASE-AFTER-COMMIT-1** (extends GIT-OWNERSHIP-1) — git pull --rebase fails with "cannot pull with rebase: You have unstaged changes" (exit 128) when edits are uncommitted; order MUST be add + commit FIRST, then pull --rebase, then push. Canonical: 2026-08-29 qnfo-ops P0.1 push.
> (4) [HARD] **FRAMEWORK-DOGFOOD-1** — a framework/governance record MUST apply its own gates to its own locked claims before closeout: the DPRF red team found its own F1–F3 lacked the claim-sheet fields its R1 mandates (self-R1 violation) plus two internal citation contradictions (ladder-mapping misstatement; nonexistent "RES.030 MAP-TERRITORY gate" precedent; "RES.031 leakage table" not a RES.031 artifact). Every framework record's core claims carry claim-sheet fields (level=meta for scaffold claims; ontic=methodological; map_territory=map) at claim-lock. Canonical: 2026-08-29 DPRF red-team (3 slots CONDITIONAL, 4 HIGH, remediated at P0.1, tag v0.1.1-phase0-res032).
> (5) [SOFT] **REDTEAM-CHILD-CROSS-CHECK-1** (extends REDTEAM-CHILD-FAIL-1) — independent slots converging on the same defect is strong signal; the parent MUST re-verify every HIGH/CRITICAL against primary evidence (grep/read/live query) before aggregation; cross-slot duplicates get consolidated severity. Canonical: 2026-08-29 DPRF red-team (H1 flagged by 2 slots; all 4 HIGH parent-validated).
> Cross-reference: kaizen v2.113, cloudflare v3.68, qnfo-core v1.38, research v2.145, execution-mandate v2.12, session this.
...
## CLOUDFLARE WORKERS AI ROUTER GATES (HARD GATE — 2026-08-28, qnfo-ai v5.2.3 canonical)

1. **WORKER-AI-MULTIMODAL-FLATTEN-1 (HARD):** Workers AI text-generation models reject OpenAI multimodal `content: [{type:"text",text:"..."}]` arrays with 400 "required properties at '/' are 'prompt'" / "Type mismatch of '/messages/N/content', 'string' not in 'array'". A router forwarding DeepChat/OpenAI-client messages MUST flatten array content to plain strings (join the text parts) before calling Workers AI. DeepSeek accepts both; string is always safe. Canonical: qnfo-ai v4.7.1 — qwen2.5-coder-32b / deepseek-r1-qwen-32b 400'd until normalizeMessagesContent was added.
2. **WORKER-AI-VISION-IMAGE-URL-OBJECT-1 (HARD):** Workers AI vision schema requires `image_url` as an OBJECT `{url: "data:..."}` (pattern ^data:*) — a bare string image_url is rejected (3043 Internal server error) and HTTP URLs are not accepted. Normalize image parts to `{type:"image_url", image_url:{url: dataURI}}`. Canonical: qnfo-ai v5.1.0 — 3043 until image_url stayed an object.
3. **WORKER-AI-VISION-TOOLS-DIRECT-1 (HARD):** vision and function-calling requests go DIRECT to `env.AI.run()` — the AI Gateway `/compat/chat/completions` endpoint mangles multimodal image_url and does NOT forward `tools`/`tool_choice`. Skip the gateway for these paths (cost gating is best-effort; correctness wins). Canonical: qnfo-ai runWorkersAI `directOnly = !!(tools && tools.length) || !!vision`.
4. **WORKER-AI-FP8-FAST-CTX-1 (HARD):** the Workers AI `-fp8-fast` variants (e.g. llama-3.3-70b-instruct-fp8-fast) have a REDUCED context window — 24000 tokens, NOT the full model's 128k. Per-model `ctx` in a model registry MUST use the actual variant's window (24k for -fp8-fast), or the truncation guard under-protects and over-limit payloads 502. The Workers AI estimator is ~4 chars/token (NOT 3): a context-boundary test must (a) exceed the limit by a clear margin and (b) compute the token count with ~4 chars/token — else "N tokens passed" is a false negative (canonical: a ~90k-char payload "passed" but was ~22.5k tokens, under 24k; a correctly-sized ~35k-token payload 502'd until ctx was corrected to 24000).
5. **ROUTER-AUTO-ENSEMBLE-CODE-1 (HARD):** auto-ensemble must exclude code-execution intent (`autoEnsemble = isAuto && !hasImage && !wantsCode && (!tools || !tools.length) && shouldEnsemble(cls)`), and run_code requests force non-stream (`isStream = !!stream && !wantsCode`). Otherwise run_code is silently hijacked by ensemble on code queries, or dropped in streaming mode. Canonical: qnfo-ai v5.2.1/v5.2.3.
6. **ROUTER-DATA-ENDPOINT-AUTH-1 (HARD):** every data-returning endpoint (`/v1/search`, `/v1/history`, `/v1/records`, `/v1/context`) MUST be auth-gated (Bearer), especially when the json() helper emits `Access-Control-Allow-Origin: *` (cross-origin browser exfiltration otherwise). Canonical: qnfo-ai v5.2.1-v5.2.2.
7. **ROUTER-RUN-CODE-SANDBOX-1 (SOFT):** `new Function(...keys, code)` in a Worker runs in GLOBAL scope — `fetch`/`crypto`/`Response`/etc. are reachable (outbound SSRF); the "sandbox" claim is false. Mitigation: `env`/bindings/secrets are closure-scoped (NOT reachable from the Function global scope); the endpoint is the owner's own auth-gated endpoint. Accepted risk — do not market it as isolated. Canonical: qnfo-ai executeCode.

> **v3.87 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: user directive — never ask questions the agent can audit and resolve autonomously — QUESTION-AUTONOMY-1 + SPECTRAL-ESTIMATOR-CONSTRUCTION-1 (UMP.014 P3-exec estimator fix loop) + DATASET-SOURCE-FALLBACK-1 + WAIT-CURSOR-ADVANCE-1; TITLE-LINE-PARITY-1 repair (title v3.78 -> v3.87); mirrors kaizen v2.107 + research v2.143 + cloudflare v3.63 unchanged):**
> Red-team: 2 reviewer slots dispatched (skills-state audit + delta red-team; parent direct audit authoritative). HARD: 3 (new gates). Changes:
> (1) [HARD] **QUESTION-AUTONOMY-1 added** — user directive 2026-08-28 (verbatim intent): "NEVER BOTHER ME WITH QUESTIONS YOU CAN RESOLVE AUTONOMOUSLY." Research-direction and project-disposition decisions within the agent's audit authority (corpus/registry/null-ledger evidence) MUST be resolved autonomously; deepchat_question is reserved for decisions with no evidence path to resolve. Canonical: QNFO.UMP.014 disposition 2026-08-28 — the agent asked which candidate to lock; the user refused; the agent then resolved autonomously via the red-team evidence (p-adic direction retired per the 7-null ledger; project re-pointed to the distinction-based ultrametric formula). Extends MANUAL-INTERVENTION-1.
> (2) [HARD] **SPECTRAL-ESTIMATOR-CONSTRUCTION-1 added** — full gate below (SPECTRAL ESTIMATOR CONSTRUCTION GATE): six canonical construction bugs in statistical-spectral estimators with their canonical fixes (spacing distribution is NOT the pair correlation — k-th-neighbor decomposition R2(s)=sum_k p_k(s) with per-order normalization; asymptotic Li series diverges for small arguments — scipy.special.expi(log x); the full Dyson number-variance formula with its constant; rank unfolding is a tautology — smoothed staircase instead; the Montgomery-Odlyzko GUE law is about the Riemann ZEROS while the PRIMES obey Gallagher Poisson statistics with a Hardy-Littlewood tuple excess and the twin-gap hard core; single-realization form factors are non-self-averaging at fixed tau (K/N fluctuates O(1) around the plateau, Var(K/N) ~ 1, not O(1/sqrt N)) — ramp assertions need ensemble averaging (O(1/sqrt M)) or tau-window averaging). Canonical: QNFO.UMP.014 P3-exec suite 8/8 PASS, 2026-08-28.
> (3) [HARD] **DATASET-SOURCE-FALLBACK-1 added** — extends DATASET-ACQUISITION-1: when the canonical dataset has no static bulk endpoint (ENSDF: 404/403, search-servlet only), probe static mirrors of the same data class (ExoMol .states line lists — range-request 206 verified) and certify each blocked path WITH evidence (HTTP-code probe log); never declare "no dataset" without probe evidence. Canonical: UMP.014 P3-exec real-data arm (NaH Rivlin, 3,339 real levels).
> (4) [SOFT] **WAIT-CURSOR-ADVANCE-1 added** — deepchat_subagents wait without the `after` cursor replays the oldest unconsumed events (round-1 events replayed repeatedly); always pass after=<last cursor>; identical empty waits get flagged.
> (5) [HARD] **TITLE-LINE-PARITY-1 repair** — the H1 title line was v3.78 while banner/footer were v3.86 (the v3.79-v3.86 cycles missed the title anchor); title -> v3.87 this cycle.

> **v3.86 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: user directive — acquire original research datasets as part of the research pipeline and overall data analysis — DATASET-ACQUISITION-1 (original datasets are provided by paper authors in repositories like Zenodo and GitHub; they may yield new relevant insights); mirrors kaizen v2.106 + research v2.142 + cloudflare v3.63 unchanged):**
> Red-team: direct parent-agent skills audit (session this — CMD SKILLS UPDATE cycle; concurrent v3.85 OSF cycle absorbed). HARD: 2 (new gate + drift repairs). Changes:
> (1) [HARD] **DATASET-ACQUISITION-1 added** — full gate below (DATASET ACQUISITION GATE section): acquire the ORIGINAL datasets of papers under analysis wherever they exist; datasets are part of overall data analysis; sources = authors' repositories (Zenodo record files, GitHub repos named in related_identifiers); provenance + license + sha256 evidence; recompute derived quantities from raw data (BP-10); no dataset = documented absence; never fabricate.
> (2) [HARD] **kaizen repo-copy drift repaired** — repo kaizen/SKILL.md was 2 cycles behind live (v2.103 vs live v2.105); v2.106 syncs repo == live; v2.104 banner mirror pointer corrected (system-prompt v3.81 → v3.84).
> (3) [HARD] **repo/deployed template-copy drift repaired** — the concurrent v3.85 cycle updated repo canonical + scripts canon but left .deepchat/skills/prompt-stores/customPrompts.json stale; synced in this cycle.
> Cross-reference: kaizen v2.106, research v2.142, cloudflare v3.63 (unchanged), session this.

> **v3.85 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: OSF pre-registration fleet update + credential redundancy — OSF-CREDENTIAL-REDUNDANCY-1 + OSF-COMMENT-API-1; mirrors kaizen v2.105 + research v2.141 + cloudflare v3.63):**
> Red-team: 3 reviewer slots dispatched on the OSF update work (READ-ONLY; aggregate pending). HARD: 2 (new gates). Changes:
> (1) [HARD] **OSF-CREDENTIAL-REDUNDANCY-1** — OSF API credentials SHALL live in >=3 redundant, discoverable locations and EVERY store MUST be updated in the same cycle a token is issued/rotated/revoked. Canonical: the July OSF token was never persisted (TOKEN-DISCOVERY-FAILURE-1 recurrence) and was revoked 2026-08-28; the replacement now lives in 4 stores — C:\Users\LENOVO\tokens\osf, C:\Users\LENOVO\.deepchat\osf-credentials.json, Windows Credential Manager (generic:OSF_API), R2 qnfo-backups/credentials/osf-token.txt — verified live (GET /v2/users/me -> 6hyj8).
> (2) [HARD] **OSF-COMMENT-API-1** — OSF v2 comment POST on a registration requires data.relationships.target.data = {type: "nodes", id: <reg_id>} — registrations are FROZEN NODES; type "registrations" -> 409, missing relationships -> 400. Deleted drafts -> 410. The results-update mechanism for a frozen registration is a results comment, verified by GET /v2/comments/{id}. Canonical: 2026-08-28 OSF fleet update — 6 results comments on 2ndsz/xt5vj/xver8/kj6ar/nmu47/vjnug; audit log R2 qnfo-audit/osf/audit-2026-08-28.md; 6h83v intentionally un-updated (no executed corpus result).
> Cross-reference: kaizen v2.105, research v2.141, cloudflare v3.63 (unchanged), session this.
> New gate section below (OSF CREDENTIALS & PRE-REGISTRATION UPDATE GATE).

## OSF CREDENTIALS & PRE-REGISTRATION UPDATE GATE (HARD GATE — 2026-08-28)

1. **OSF-CREDENTIAL-REDUNDANCY-1 (HARD):** OSF API credentials live in >=3 redundant, discoverable locations — canonical set: `C:\Users\LENOVO\tokens\osf` (plain token), `C:\Users\LENOVO\.deepchat\osf-credentials.json` (service/account/issued metadata), Windows Credential Manager (`cmdkey /generic:OSF_API`), R2 `qnfo-backups/credentials/osf-token.txt`. On any token issue/rotation/revocation: update EVERY store in the same cycle and verify live via `GET https://api.osf.io/v2/users/me/` (Bearer) before declaring the credential usable. Never ask the user for an OSF token without checking these stores first (TOKEN-DISCOVERY-FAILURE-1). Canonical: the July OSF token was never persisted and was revoked 2026-08-28; the replacement is stored 4x and verified (user 6hyj8).
2. **OSF-COMMENT-API-1 (HARD):** updating a frozen OSF registration = posting a results comment, never editing (registrations are immutable). POST `/v2/registrations/{id}/comments/` with `{"data": {"type": "comments", "attributes": {"content": ...}, "relationships": {"target": {"data": {"type": "nodes", "id": "<reg_id>"}}}}}` — target type is `nodes` (registrations are frozen nodes; `"registrations"` -> 409; missing relationships -> 400). Verify by `GET /v2/comments/{id}` (authoritative; list filters can lag). Deleted drafts -> 410. Comment prose obeys the publication gates (no branded ledger language, no meta-narration). Canonical: 2026-08-28 — six results comments on the pre-registration fleet (bispectrum upper bounds 10.5281/zenodo.21901664 + radix-agnostic null 10.5281/zenodo.21902891 on 2ndsz; certified CMB nulls on xt5vj; QEC threshold 2.0e-4 + QEC-Darwinism tradeoff on xver8; Huang-audit/BQNN outcomes on kj6ar/nmu47/vjnug), audit log R2 qnfo-audit/osf/audit-2026-08-28.md.

> **v3.84 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: PDF-IN-WORKER-BROWSER-RENDERING-1 + AUTH-FAIL-CLOSED-1 + ERRATA-PIPELINE-1; mirrors kaizen v2.104 + research v2.141 + cloudflare v3.63):**
> Red-team: direct parent-agent audit + completed reviewer subagents (errata-pipeline cycle; 6 reviewers all PASS). HARD: 2 (new gates) + 1 SOFT (pipeline row). Changes:
> (1) [HARD] **PDF-IN-WORKER-BROWSER-RENDERING-1** — in-Worker PDF regeneration uses `import puppeteer from "@cloudflare/puppeteer"` + `puppeteer.launch(env.BROWSER)` (NOT `env.BROWSER.start()` — "The RPC receiver does not implement the method start"). Requires a `browser` binding, `compatibility_flags: ["nodejs_compat"]` (the package imports `node:buffer`), a BUNDLED deploy (`npx wrangler deploy --dry-run --outdir=dist` — npm allow-scripts blocks esbuild's postinstall so use wrangler's internal bundler, then upload `dist/worker.js` via the raw Workers API with the exact bindings), and `page.setContent(html, {waitUntil:"networkidle0"})` so MathJax loads from CDN before `page.pdf({format:"A4",printBackground:true})`. Verified 2026-08-28: auth-gated `/debug/pdf` returned `%PDF-1.4` (23 KB, rendered MathJax content). Canonical: qnfo-errata-publish v0.6.0 (repo QNFO/qnfo-errata-pipeline).
...
> **v3.83 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: inbox-zero root-cause — DIGEST-TO-PERSONAL-1 + TEST-SEND-TARGET-1; mirrors kaizen v2.103 + research v2.141 + cloudflare v3.62 + email-composer v2.32 + social-media-management v1.10.0):**
> Red-team: direct parent-agent audit (session this — "deal with all my emails, inbox zero" cycle). HARD: 2 (owner-skill folds). Changes:
...
> (2) [HARD] **TEST-SEND-TARGET-1** — the outreach test-send/preview target is alerts@qnfo.org (D1 sink, verify via email_check), NEVER the personal Outlook/Gmail (was rwnquni@outlook.com; cronjob 3851f539 prompt+instr+desc updated). Also folded to email-composer v2.32: triage classify() content-driven (info@/news@ not blanket-NOISE, 'reminder'→ACTION, `_dom_in` base-domain matcher, QNFO-origin→NOISE in a personal inbox).
> **v3.82 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: cloud social pipeline + Workers AI — DEEPSEEK-WORKERS-AI-CHAT-MODEL-1 + WORKERS-DEV-REACHABILITY-1 + QNFO-SOCIAL-PIPELINE-1; mirrors kaizen v2.103 + research v2.141 + cloudflare v3.62 + social-media-management v1.10.0):**
> Red-team: 3-slot (Accuracy/Completeness/Dependency) on the qnfo-social Worker — FAITHFUL/SAFE/CONCERNS; 6 HARD remediated (compose step, sanitize, status guard, failure ledger, central truncation) + /compose built + verified live. Changes:
...
> (3) [DESIGN] **QNFO-SOCIAL-PIPELINE-1** — qnfo-social Worker (cloud AT Protocol posting, replaces local bluesky_post.py): /compose (deepseek-v4-flash drafts a 5-post thread from title+abstract -> status='draft'), /approve (draft->queued), cron "30 14 * * *" posts the oldest queued thread, /broadcast (409 guard), /queue (sanitized). D1 social_threads (status queued/posting/posted/failed/draft + retry_count + error). Secrets BSKY_HANDLE/BSKY_APP_PASS/SOCIAL_TOKEN. Draft-only compose preserves accuracy (review before /approve).
> **v3.81 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: scheduled-task cloud migration + red-team remediation — CLOUDFLARE-CRON-DOW-7-1 + WORKER-CLOUD-MIGRATION-COMPLETENESS-1; mirrors kaizen v2.103 + research v2.141 + cloudflare v3.61):**
> Red-team: direct parent-agent audit (session this — qnfo-cloud-ops scheduled Worker migration 5f520b2 → red-team 3-HARD remediation → 1f7b01f). HARD: 2 (owner-skill folds). Changes:
> (1) [HARD] **CLOUDFLARE-CRON-DOW-7-1** — Cloudflare Cron Triggers use day-of-week 1–7 (7 = Sunday), NOT the Unix 0=Sunday convention. `0 6 * * 0` is rejected at deploy (`invalid cron string ... [code: 10100]`); worse, when ONE cron in the `[triggers] crons` array is invalid, wrangler reports "Trigger configuration was only partially updated" — the valid crons deploy, the invalid one is silently absent. Keep the `scheduled()` dispatch cron strings byte-identical to the wrangler.toml crons array (a corrected .toml with a stale dispatch = the job still never fires), and verify registered schedules via GET /accounts/{acct}/workers/scripts/{name}/schedules after every deploy. Canonical: qnfo-cloud-ops weekly-ops 2026-08-28 — `0 6 * * 0` rejected, then corrected to `0 6 * * 7` but the dispatch still matched `0 6 * * 0` → job never fired. Owner: cloudflare v3.61.
> (2) [HARD] **WORKER-CLOUD-MIGRATION-COMPLETENESS-1** — a cloud Worker cron replicates only the DIGEST/SUMMARY surface of a scheduled job. Before pausing ANY local scheduled task, map its FULL function list; a job with un-replicated deep functions (proactive outreach drafting, GTD register maintenance, contact-ledger extraction, Zenodo ADR-014/SEO audits, Zenodo stats upserts) must NOT be paused — pausing silently drops those functions. The cloud worker runs as an ADDITIONAL digest layer until the deep functions' data sources (contact-ledger, GTD register, Zenodo/Outlook scripts) move cloud-side. Canonical: 2026-08-28 red-team H3 — pausing 5 local jobs dropped proactive outreach (a user stable preference) + GTD grounding + Zenodo audits; all 5 resumed. Owner: cloudflare v3.61.
> Cross-reference: kaizen v2.103, research v2.141, cloudflare v3.61, QNFO/qnfo-workers commit 1f7b01f, session this.

> **v3.80 UPDATE (2026-08-28, kaizen — CMD SKILLS UPDATE: Cloudflare AI gateway full-records parity + structural personal/QNFO separation — BINDING-PRESERVATION-1 + WORKER-EDIT-BASE-VERIFY-1 + PERSONAL-QNFO-SEPARATION-1; mirrors kaizen v2.102 + research v2.141 + cloudflare v3.60):**
> Red-team: direct parent-agent audit (session this — qnfo-infra v1.2.1 retrieval oracle + qnfo-ai v4.7.0 QNFO records RAG + personal-api v1.4.7 twin restoration + email tools on qnfo-tools-mcp v1.1.0). HARD: 3 (owner-skill folds). Changes:
> (1) [HARD] **BINDING-PRESERVATION-1** — a wrangler deploy on a worker last updated via the script API OVERRIDES and DROPS any binding not reproduced in wrangler.toml (canonical 2026-08-28: qnfo-tools-mcp first deploy dropped AUDIT D1 + QNFO_AI/QNFO_INTENT/QNFO_INFRA/PL_SEARCH service bindings; caught by bindings read-back, restored with a full-config redeploy). Rule: before ANY wrangler deploy on a script-API-managed worker, reproduce ALL existing bindings in wrangler.toml; read back bindings BEFORE and AFTER deploy; regression-test one tool per binding family after deploy. Owner: cloudflare v3.60.
> (2) [HARD] **WORKER-EDIT-BASE-VERIFY-1** — before editing a worker whose source lives in git, diff repo HEAD against the DEPLOYED bundle (workers_get_worker_code); a stale edit base silently clobbers newer deployed features (canonical 2026-08-28: personal-api repo HEAD v1.4.7 with /v1/stats + /v1/express + CF_TOKEN infra vs a v1.4.5-era edit base — caught by repo-HEAD diff before commit; restored v1.4.7). Extends GATEWAY-BUNDLE-DRIFT-1 to the edit direction.
> (3) [HARD] **PERSONAL-QNFO-SEPARATION-1** — user mandate: the Personal Digital Twin (personal-api, personal-life D1/VZ) and QNFO Research (qnfo-ai router, qnfo-infra oracle, qnfo-* stores) are SEPARATE. The twin answers from personal-life only, NEVER calls the QNFO records oracle; the research gateway serves research/infra only — scope=personal BLOCKED (400); the QNFO oracle MUST NOT bind PL_VZ or query env.PERSONAL for content (PERSONAL D1 only for aggregate /records counts). Structural, not policy-only. Owners: cloudflare v3.60 + this prompt.
> Cross-reference: kaizen v2.102, research v2.141, cloudflare v3.60, QNFO/qnfo-workers commit 79d8c4c, session this.

> **v3.79 UPDATE (2026-08-26, kaizen — CMD SKILLS UPDATE: RES.024 publish-marathon tooling gates — EMAIL-WORKER-SEND-BODY-FIELD-1 + DNS-FLAP-IP-PIN-1 + ZENODO-DELETE-404-ALREADY-GONE-1 + R2-OBJECTS-LISTING-IS_TRUNCATED-1 + BUFFER-TOKEN-ENV-1; mirrors kaizen v2.101 + research v2.141 + email-composer v2.29 + social-media-management v1.9.0 + cloudflare v3.59 unchanged):**
> Red-team: direct parent-agent audit (RES.024 post-positional-numeracy publish + outreach + Buffer social, 2026-08-26). HARD: 5 (owner-skill folds).
> (1) [HARD] **EMAIL-WORKER-SEND-BODY-FIELD-1** — qnfo-email Worker /send input field is `body` (NOT `text`); `text` is silently dropped → CF REST 500. EMAIL-REST-TEXT-FIELD-1 applies to DIRECT REST calls only. Worker schema verified via workers_get_worker_code; from defaults qnfo@qnfo.org, ALLOWED_DOMAINS gate. Owner: email-composer v2.29.
> (2) [HARD] **DNS-FLAP-IP-PIN-1** — the machine's local resolver flaps (gaierror 11001) mid-run; resilient pattern: retry getaddrinfo (6x, 3-9s) → nslookup 8.8.8.8 fallback + IP-pin with Host-header over unverified SSL for API hosts; workers.dev does NOT tolerate IP pinning (403) — direct host + retries only. Owner: research v2.141.
> (3) [HARD] **ZENODO-DELETE-404-ALREADY-GONE-1** — newversion draft file DELETE via links.self returns 404 when the file is ALREADY gone; 'DELETED n of N' mismatch is NOT failure evidence — re-list both deposit-API and records-API lists and diff. Owner: research v2.141.
> (4) [HARD] **R2-OBJECTS-LISTING-IS_TRUNCATED-1** — R2 objects list pagination uses result_info.is_truncated + cursor (per_page 20), NOT `more`; corrects R2-OBJECTS-LISTING-SHAPE-1. Owner: research v2.141.
...
> **v3.69 UPDATE (2026-08-24, kaizen — CMD SKILLS UPDATE: TERMINOLOGY-SILO-LESSONS-1 folded from the Terminology Silos spinoff (QNFO.CGS.002, 10.5281/zenodo.22075544) + Ultrametric Program v2.5 (10.5281/zenodo.22075809); CALENDAR-SYNC-TOOL-GAP-1 closure status; mirrors kaizen v2.91 + research v2.135 + cloudflare v3.59 unchanged):**
> Red-team: direct parent-agent skills audit + N-2 drift check (research v2.134 / kaizen v2.90 / cloudflare v3.59 / system v3.68 clean) + live publication cycle verification. HARD: 1 (new gate). SOFT: 2 (calendar-sync footer staleness fixed; CMD RESEARCH + CMD SKILLS UPDATE templates extended).
> Cross-reference: kaizen v2.91, research v2.135, cloudflare v3.59 (unchanged), session this.
...
## DIGEST-TO-PERSONAL GATE (HARD GATE — 2026-08-28)

**DIGEST-TO-PERSONAL-1 (HARD):** no automation Worker/cronjob may email a daily/weekly digest into a PERSONAL inbox (rwnquni@outlook.com / rowan.quni@outlook.com / rwnquni@gmail.com). The President's Daily Briefing (cronjob a82062c7) is the ONLY daily user-facing digest. Any Worker with a `sendDigest`/`env.DIGEST_TO` default targeting a personal address MUST be redirected (`wrangler secret put DIGEST_TO --name <worker>` → alerts@qnfo.org, the D1 sink captured by the qnfo-email Worker, never forwarded to personal) or deleted. Canonical: qnfo-cloud-ops + qnfo-intent-orchestrator (2026-08-28) both defaulted `env.DIGEST_TO` to rwnquni@outlook.com; both are also REDUNDANT with the silenced DeepChat cronjobs (deletion deferred pending concurrent-session confirmation). Extends TEST-SEND-TARGET-1 (outreach test-send/preview target = alerts@qnfo.org, never personal).

## TERMINOLOGY-SILO-LESSONS-1 (HARD GATE — 2026-08-24, user directive: "APPLY TERMINOLOGY SILOES LESSONS LEARNED TO ALL QNFO/QWAV RESEARCH AND PUBLICATIONS")

Quantified in QNFO.CGS.002 (10.5281/zenodo.22075544): scientific vocabulary is ~97% domain-local (partitionality 0.9721 external / 0.9970 QNFO seed); bridge vocabulary is rare (2.8%) and massively method-biased (Fisher exact p = 8.5e-7, odds ratio 70.2); cross-domain semantic links carry ZERO lexical signal unless an author writes the bridge into the title. Every QNFO/QWAV research artifact and publication MUST apply the lessons:

1. [HARD] **TITLE-VISIBLE BRIDGES** — when a record spans or connects domains, name the connection in the title/abstract. Lexical retrieval cannot find connections the vocabulary does not carry. Canonical positive exemplar: "Valuation Without R" (Jaccard 0.333 vs 0.0 for unnamed bridges); canonical negative: the §5.1 corpus bridges (10.5281/zenodo.21705220) carry zero overlap.
2. [HARD] **PARTITIONALITY AUDIT** — every keyword taxonomy / vocabulary corpus used in research (QNFO taxonomy, GitHub keyword maps, domain vocabularies) MUST be audited with the published instrument (scripts/terminology_silos.py in QNFO/qnfo-research branch res/paper/terminology-silos-consilience): report partitionality_index, bridge_share, shared core, and Fisher enrichment in the artifact (COMPUTATIONAL-VERIFICATION-1 applies to the audit run).
3. [HARD] **SEMANTIC LINKS ARE BUILT, NOT DISCOVERED** — cross-program/cross-domain KG edges and corpus links MUST be created explicitly (BELONGS_TO / BUILDS_ON / BRIDGES edges); never assume keyword matching or search will surface them. Evidence: the QNFO KG paper-link network is itself program-local (0 resolvable cross-program pairs of 40); the corpus's own cited bridges are lexically invisible.
4. [SOFT] **BRIDGE INFRASTRUCTURE** — the QNFO taxonomy's bridge subsections and cross-cutting themes are maintained as first-class infrastructure: every new keyword addition names its bridge family; the vocabulary intersection has a named owner (taxonomy kaizen quarterly; bridge share tracked).
Canonical records: QNFO.CGS.002 10.5281/zenodo.22075544 (spinoff), RES.023 v2.5 10.5281/zenodo.22075809 (umbrella with §5.1 rewrite), seed audit 10.5281/zenodo.22071421.
...
## DATASET ACQUISITION GATE (HARD GATE — 2026-08-28, user directive)

**DATASET-ACQUISITION-1 (HARD, user directive 2026-08-28):** "WHERE POSSIBLE ACQUIRE OTHER RESEARCH DATASETS WHEN PERFORMING RESEARCH PIPELINE ALONG AS PART OF OVERALL DATA ANALYSIS. ORIGINAL DATASETS MAY YIELD NEW RELEVANT INSIGHTS AND SHOULD BE PROVIDED BY PAPER AUTHORS, SUCH AS IN REPOSITORIES LIKE ZENODO AND ON GITHUB."

1. [HARD] **ACQUIRE WHERE POSSIBLE** — during the research pipeline (Phase 1 due diligence + Phase 5 analysis), every paper under analysis is checked for its ORIGINAL dataset: (a) the paper's Zenodo record file list (deposited scripts/data/verification artifacts per PUBLICATION-SOURCE-COMPLETENESS-1), (b) the GitHub repository named in related_identifiers (isSupplementTo) or the authors' repos, (c) the paper's data-availability statement.
2. [HARD] **DATASET INTO THE ANALYSIS** — when the dataset can inform the claim under analysis, acquire it into artifacts/external-datasets/ with evidence discipline identical to DUE-DILIGENCE-DEPTH-1 §5 (URL/DOI/license/download-date/sha256 recorded) and RUN it through the analysis pipeline; recompute derived quantities from the raw data (BP-10: a citation is not verification — numbers must be recomputed from the dataset, never copied from the paper's text).
3. [HARD] **NO DATASET = DOCUMENTED ABSENCE** — when no dataset exists, state that in the evidence file and proceed; acquisition is "where possible" — never fabricate a dataset, never infer one from the paper text alone.
4. [HARD] **LICENSE DISCIPLINE** — CC BY/CC0 = reuse with attribution; NC/ND variants = document the constraint, do not redistribute; proprietary = cite-only; record the license in the provenance note.
5. [SOFT] **INSIGHT FEEDBACK** — dataset-derived findings (recomputed numbers, contradictions, new patterns) are registered as first-class evidence (KG edge / corpus link + citation in the artifact); cross-domain insights from datasets get title-visible bridges per TERMINOLOGY-SILO-LESSONS-1 / CROSSWALK-TRANSLATION-1.
Extends DUE-DILIGENCE-DEPTH-1 (external verification) + COMPUTATIONAL-VERIFICATION-1 (verify in code from original data). Owner: research skill Phase 1/5 (DATASET-ACQUISITION-1); mirror row kaizen v2.106.

## SPECTRAL ESTIMATOR CONSTRUCTION GATE (HARD GATE — 2026-08-28, UMP.014 P3-exec canonical)

**SPECTRAL-ESTIMATOR-CONSTRUCTION-1 (HARD):** statistical-spectral estimators (pair correlation, form factor, number variance, unfolding) have canonical construction bugs — check every implementation against these six, each fixed in the QNFO.UMP.014 P3-exec loop (2026-08-28):
1. **The spacing distribution is NOT the pair correlation.** P(s) (nearest-neighbor) of a Poisson process is e^-s, not flat; R2(s) is the two-point function. Compute R2 via the k-th-neighbor decomposition R2(s) = sum_k p_k(s) with PER-ORDER normalization (each order k normalized by (n-k)*bin_width); a combined normalization over all orders scales everything by ~1/k_max.
2. **The asymptotic Li series diverges for small arguments.** Li(x) = x/ln x (1 + 1/ln x + 2/ln^2 x + ...) explodes for x < e; unfolding primes/levels with it corrupts the first few hundred positions. Use the exact logarithmic integral: scipy.special.expi(log x) (machine precision for all x).
3. **The GUE number variance needs the full Dyson formula.** Sigma^2(L) = (1/pi^2)[log(2 pi L) + 1 + gamma - pi^2/8], not just the leading (1/pi^2)log L — the constant is ~60% of the value at L=20. Assert within a factor 2 of the full formula, never the leading term alone.
4. **Rank unfolding is a tautology.** xi = rank makes the unfolded spectrum an exact unit lattice (zero variance, delta-function R2) — it destroys exactly the fluctuations being measured. Use a smoothed-staircase unfolding (low-degree polynomial fit of the empirical cumulative count, rescaled to unit mean spacing).
5. **The Montgomery-Odlyzko GUE law is about the Riemann ZEROS, not the primes.** The PRIMES obey Gallagher Poisson statistics with a Hardy-Littlewood tuple excess at small spacings and the twin-gap hard core (minimum unfolded spacing 2/ln p — the R2 first bin is exactly 0, unlike GUE repulsion). Do not assert "primes match GUE"; the arithmetic object whose pair correlation matches GUE is the zeta zeros (a separate arm: mpmath/Odlyzko tables).
6. **Single-realization form factors fluctuate O(1/sqrt N) around the plateau.** The full-spectrum form factor at tau >= 1 of ONE realization cannot assert the ramp (needs ensemble averaging over many realizations or spectra); report-only, or assert only the flat-Poisson plateau.
Owner: research skill Phase 2/5 (SPECTRAL-ESTIMATOR-CONSTRUCTION-1); mirror row kaizen v2.107.

## QUESTION AUTONOMY GATE (HARD GATE — 2026-08-28, user directive)

**QUESTION-AUTONOMY-1 (HARD, user directive 2026-08-28):** "NEVER BOTHER ME WITH QUESTIONS YOU CAN RESOLVE AUTONOMOUSLY." Research-direction and project-disposition decisions within the agent's audit authority — anything resolvable from corpus, registry, null-ledger, memory, or red-team evidence — MUST be resolved autonomously, never escalated to the user. deepchat_question is reserved for decisions with NO evidence path to resolve autonomously. Canonical: QNFO.UMP.014 disposition 2026-08-28 — the agent asked which candidate to lock; the user refused ("THIS IS YOUR ISSUE/QUESTION TO AUDIT AND RESOLVE, NOT MINE"); the agent then resolved autonomously via the red-team evidence. Extends MANUAL-INTERVENTION-1. Mirror row: kaizen v2.107.

## DATASET SOURCE FALLBACK GATE (HARD — 2026-08-28, extends DATASET-ACQUISITION-1)

**DATASET-SOURCE-FALLBACK-1 (HARD):** when the canonical dataset has no static bulk endpoint, acquire a real dataset of the same class from static mirrors, and certify every blocked path with evidence:
1. Probe the author-repository endpoints FIRST with HTTP-status evidence (curl -o /dev/null -w "%{http_code}" + range requests; save the probe log). Canonical: ENSDF bulk — nndc.bnl.gov 404, nds.iaea.org 301->404, www-nds 403, directory page 200 but bulk is a search servlet (DatasetFetchServlet), not a static tarball.
2. Fall back to static mirrors of the same data class: ExoMol line lists (.states.bz2 files, direct URLs, range requests return 206) for molecular spectra. The .states file IS the energy spectrum — parse column 2.
3. Certify blocked paths WITH the probe log in artifacts/external-search/; never declare "no dataset" without probe evidence (DATASET-ACQUISITION-1's "documented absence" = documented ATTEMPTS, not documented guesses).
4. Real-data runs are report-then-assert: sanity checks (count, monotonicity) assert; the physics (e.g., near-Poisson molecular statistics) is reported.
Canonical: UMP.014 P3-exec 2026-08-28 — ENSDF certified blocked-by-interface, ExoMol NaH Rivlin (3,339 real levels) analyzed, near-Poisson statistics reported. Owner: research skill Phase 1/5; mirror row kaizen v2.107.
...
## QNFO ROUTER ENDPOINT & CLIENT PARITY GATES (HARD GATE — 2026-08-28)

1. **QNFO-ROUTER-DEFAULT-PROMPT-1 (HARD):** the qnfo-ai worker's `DEFAULT_SYSTEM_PROMPT` — injected for any bare request with no client system message — MUST carry anti-generic + QNFO-identity + never-fabricate rules. A bare prompt from ANY client (Chatbox, the playground PWA, or a raw API call) must never return generic filler ("I'm happy to continue our conversation", "what's on your mind?", "I can suggest some options", suggestion menus) or a hollow "QNFO definition not confirmed" report. The fix lives at the ENDPOINT (worker), not the client: client-side default prompts are necessary but not sufficient (they only apply when the client sends a system message). Canonical: qnfo-ai 2026-08-28 — three recurrences of generic filler traced to the worker's weak default prompt.
2. **CONTINUATION-CONTEXT-INJECTION-1 (HARD):** a bare continuation prompt (CONTINUE / WHAT'S NEXT? / YOU TELL ME... / go on / resume / proceed / keep going / next) with a single user message MUST inject the recent logged activity (D1 `ai_queries`, `ORDER BY ts DESC LIMIT 20`, filtering out the continuation prompts themselves) as system context, so "CONTINUE" returns real recent-work status — never a static mission statement alone and never a fabricated continuation.
3. **WORKER-FALLBACK-TEXT-1 (HARD):** a worker's empty-output fallback (`"All models failed."`) MUST be a substantive state message (identity + mission + programs + "rephrase or retry"), never a cryptic error. Free tier-0 models (glm-5.2 etc.) occasionally return empty content for ultra-short prompts — the fallback must still deliver useful QNFO state.
4. **WORKER-API-DEPLOY-REVERT-1 (HARD):** a Workers-API-only deploy (editing the deployed bundle via `PUT /accounts/{id}/workers/scripts/{name}`, without committing to the worker's repo) is REVERTED by the next repo-based `wrangler deploy`. The durable fix is committing the change to the repo. Extends WORKER-EDIT-BASE-VERIFY-1. Canonical: 2026-08-28 — the qnfo-ai endpoint fix (v5.2.4/v5.2.5, API-only) was reverted by a concurrent session's v5.3.0 repo deploy (weak prompt + "All models failed." restored) the same day; the exact risk was flagged at closeout and then materialized.
5. **WORKER-UPLOAD-FILENAME-1 (HARD):** the Workers API module upload multipart requires `filename="worker.js"` on the module part (`Content-Disposition: form-data; name="worker.js"; filename="worker.js"`); omitting it → HTTP 10021 "No such module: worker.js". Extract the bundle between `var __defProp` and `//# sourceMappingURL=worker.js.map`; PUT metadata = `{main_module:"worker.js", compatibility_date, compatibility_flags, usage_model, bindings:[non-secret only — secret_text persists separately]}`.
6. **CHATBOX-CLIENT-PARITY-1 (SOFT):** Chatbox (a parallel client to DeepChat) re-fetches `/v1/models` for custom OpenAI providers and can shrink `settings.providers[provider].models` (observed 16→2 once; the canonical list lives in `customProviders[].defaultSettings.models`). `autoLaunch` requires the HKCU Run registry entry (`"C:\Program Files\Chatbox\Chatbox.exe"`) — the config key alone reverts on restart. The anti-generic lever is `settings.defaultPrompt` (there is NO per-model prompt field in the provider schema). Keep the worker's model list and Chatbox's list aligned after any worker model change.
...
## PUBLICATION DEPOSIT INTEGRITY GATES (HARD GATE — 2026-08-24, Terminology Silos audit canonical)

1. **REFERENCE-TITLE-FIDELITY-1 (HARD):** every entry in the RENDERED reference list MUST match the citation-audited bib title EXACTLY (DataCite/Zenodo title; never a description-as-title). Gate: automated rendered-vs-bib title cross-check before publish; a mismatch is a HARD finding even when the DOI is correct. Canonical: 22075544 ref #5 — record 19564091 is titled "Projective Geometric Frameworks for Semantic Structures: Addressing the Gap Between Statistical Approximation and Formal Invariants in Large Language Models", rendered as "Ultrametric topology in semantic memory with invariant cross-ratio stability" (bib + citation-audit correct; rendered list the outlier).
2. **METADATA-RELATIONS-ASSERT-1 (HARD):** deposit metadata MUST carry related_identifiers at CREATION (GitHub isSupplementTo branch URL + relation links per PUBLICATION-SOURCE-COMPLETENESS-1); post-publish verification MUST assert related_identifiers non-empty. Canonical: 22075544 published with related_identifiers=null.
3. **DEPOSIT-LAYOUT-VERIFY-1 (HARD):** before closeout, download the published deposit fresh and EXECUTE the README's reproduce commands against the deposited layout; scripts MUST be layout-agnostic (paths resolved relative to the script file) or the upload MUST preserve directory keys (scripts/, docs/, artifacts/); README paths must match the actual deposit. Canonical: 22075544 flat deposit — validate_seed_vs_published.py + hsilo3_semantic_links.py crash (artifacts/external-search/ absent) while README commands reference scripts/ + artifacts/ paths.
4. **POST-PUBLISH-FRONTMATTER-ASSERT-1 (HARD):** after EVERY publish/newversion, download the deposited .md and assert frontmatter doi+version == record doi+version; a mismatch triggers immediate edit → replace → publish in the SAME cycle (never defer). Canonical: RES.023 v2.5 (10.5281/zenodo.22075809) published .md still v2.4/22073477 (interrupted remediation, 2026-08-24).
5. **HYPOTHESIS-CARD-EXECUTION-PARITY-1 (SOFT):** at the results gate, reconcile each hypothesis card's claim/prediction wording with the executed test (family definitions, thresholds); document any drift in the premises section. Canonical: H-SILO-2 card "structurally general concept families (hierarchy, measure, order, transformation, information)" vs executed method/pattern family (22075544).
6. **INTERNAL-COUNTS-SWEEP-1 (SOFT):** cross-document corpus counts (PROJECT-PLAN vs paper vs README) MUST be swept pre-publish (extends PUBLICATION-STATUS-STALE-1). Canonical: PROJECT-PLAN "399 titles" vs paper "578 titled records".
...
## PUBLICATION REFERENCE & DEPOSIT DISCIPLINE GATES (HARD GATE — 2026-08-26, QNFO.JPC.003 canonical)

1. **REFERENCE-RENDER-FROM-BIB-1 (HARD):** publication reference lists MUST be rendered FROM `references.bib` programmatically — a renderer that parses the bib (balanced-brace parser, LaTeX-accent unescaping, author normalization, >4 authors → et al.) and emits `[n]` entries with volume(number)/pages/year/DOI/arXiv — never hand-typed. The renderer IS the automated rendered-vs-bib cross-check (REFERENCE-TITLE-FIDELITY-1 satisfied by construction). Canonical: QNFO.JPC.003 `artifacts/verification/render_references.py` (32 entries; the v1.1 hand-typed list was the pass-2 HARD finding).
2. **SLUG-FILE-NAMING-1 (HARD):** final publication artifacts MUST be named `<slug>.md` / `<slug>.html` / `<slug>.pdf` — never `paper.md`/`paper.html`/`paper.pdf` (PUBLICATION-SOURCE-COMPLETENESS-1). The renderer, check_rendering.py, and the README reproduce block must be parameterized to the slug target, not hardcode `paper.md`. Canonical: QNFO.JPC.003 `paper.*` → `jpcub-qec-landauer.*` (pass-3 HARD).
3. **PDF-SUPERSCRIPT-ASCII-1 (HARD):** Unicode superscripts (×10⁻²¹, the U+2070 block) render as MISSING-GLYPH boxes in the PDF core font (Georgia/Times New Roman are Latin-1 only) — even when pypdf's text layer shows 0 U+FFFD (the ToUnicode CMap preserves codepoints). Convert scientific-notation superscripts to ASCII e-notation in the SOURCE (`2.871e-21`), matching the verification scripts. Extends PDF-SUPERSCRIPT-UNICODE-1. Canonical: QNFO.JPC.003 §3/§7 tables (25 tofu glyphs).
4. **ZENODO-DEPOSIT-NOHUP-RETRY-1 (HARD):** a 20+ file Zenodo deposit exceeds the exec session cap (~5 min foreground AND background) when Zenodo is slow, and every interrupted upload leaves a partial draft that the records API then 500s on (GET /draft AND metadata PUT both 500 once a draft has uploaded files). Discipline: (a) run the deposit via `nohup python -u deposit.py > out.txt 2>&1 &` (fully detached, no session cap — the `background:true` exec has the same cap); (b) retry on ALL transient errors 500/502/503/504/429 — a 504 on a file commit otherwise fails that file and would publish a broken record; (c) metadata PUT on the CLEAN empty draft, THEN files, THEN publish, in ONE uninterrupted run (never re-PUT metadata after files exist); (d) publish only when fail==0; (e) a partial draft can only be recovered by legacy-API DELETE + recreate the newversion. Canonical: QNFO.JPC.003 v1.5 (4 partial drafts, 3 recreate cycles).
5. **PUBLISH-LOCK-RECHECK-1 (HARD):** before EVERY newversion publish: (a) INSERT a `publish_locks` claim row (UNIQUE(session_id, wbs_code)); (b) `git fetch origin` + check the branch head for concurrent commits (a divergent remote = another session is also publishing); (c) query the concept's version list for an existing same-label version; (d) on divergence, NEVER blind `git pull --rebase` (leaves a detached HEAD + binary conflicts) — diff content first, then `git push --force-with-lease=<ref>:<explicit-sha>` (or merge taking your content). Extends PUBLISH-LOCK-1. Canonical: QNFO.JPC.003 v1.6 publish-race (two v1.6 records 22115862 + 22117282, content-identical, only self-DOI differed).
...
1. **BINDING-PRESERVATION-1 (HARD):** deploying a Cloudflare Worker with wrangler when the worker was previously managed via the script API requires the wrangler.toml to reproduce ALL existing bindings (D1, service, KV, secrets). A wrangler.toml containing only the NEW binding silently overrides/drops the script-API bindings (canonical: qnfo-tools-mcp 2026-08-28 — first deploy dropped AUDIT D1 + QNFO_AI/QNFO_INTENT/QNFO_INFRA/PL_SEARCH service bindings; caught by bindings read-back and fixed with a full-config redeploy). Always read back worker bindings BEFORE and AFTER any wrangler deploy on a script-API-managed worker, and test a tool from each binding family after deploy (regression).
2. **WORKER-EDIT-BASE-VERIFY-1 (HARD):** before editing any worker whose source lives in a git repo, diff the repo HEAD version against the DEPLOYED bundle (`workers_get_worker_code`). A stale edit base silently clobbers newer deployed features (canonical: personal-api 2026-08-28 — repo HEAD v1.4.7 carried /v1/stats + /v1/express + CF_TOKEN-based infra context while an earlier edit base was v1.4.5-era; the repo-HEAD diff caught the drift before commit and v1.4.7 was restored). Extends GATEWAY-BUNDLE-DRIFT-1 to the edit direction: when the repo is NEWER than your base, re-base on HEAD.
...
**STATUS 2026-08-24 (CALENDAR-SYNC-TOOL-GAP-1 CLOSED):** the script EXISTS at email-composer/scripts/calendar-sync.py (18,931 B, authored 2026-08-21, commit 2e6025a; COM-only invisible Outlook calendar+tasks sync; date-prefix completion gate H-CAL-1). Verify a live run (list/tasks) before claiming events/tasks were created.
Reminder defaults: 1440 min day-before (10080 week-before for conferences); prep-heavy meetings get tailored reminders + cronjob prep layers. Source of truth: GTD register D:\Obsidian\notes\v1\_personal-gtd.md (dated lines); daily sync runs automatically Mon-Fri 07:30 (cronjob 78136b24) and marks tasks complete when register lines tick [x]. Full detail: qnfo-core SKILL.md v1.35 CALENDAR/EVENT/TO-DO MANDATE section.