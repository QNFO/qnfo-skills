> **v3.94 UPDATE (2026-08-29, kaizen — CMD SKILLS UPDATE: DPRF v0.2 newversion publish gates — NEWVERSION-DRAFT-FILE-KEY-1 + ZENODO-BUCKET-PUT-CANONICAL-1 + ZENODO-NEWVERSION-STRAY-PURGE-1 + ZENODO-CONCEPTRECID-COERCE-1 + BUILD-PDF-BIB-FILENAME-1 + CITE-AUDIT-LIVE-API-1; FRONTMATTER-DUPLICATION-1 reinforced (RES.032 v0.1); mirrors kaizen v2.114 + research v2.146 + cloudflare v3.69 + qnfo-core v1.39 + execution-mandate v2.13):**
> Red-team: direct parent-agent audit (session this — QNFO.RES.032 v0.1→v0.2 newversion saga; 4-slot formatting gate + subagent fix-verification of the deposited record, all delivered). HARD: 5. SOFT: 1. Changes:
> (1) [HARD] **NEWVERSION-DRAFT-FILE-KEY-1** — Zenodo newversion draft file entries may use `filename` instead of `key`; always read via `f.get("key") or f.get("filename")` when listing/deleting draft files (canonical: 2026-08-29 RES.032 v0.2 — KeyError on f["key"] in the carried-file delete loop).
> (2) [HARD] **ZENODO-BUCKET-PUT-CANONICAL-1** (extends ZENODO-DEPOSIT-DELETE-500-1) — POST /api/deposit/depositions/{id}/files can return 400 on a newversion draft after carried-file deletion; the reliable upload path is the bucket: PUT {bucket}/{name}?access_token=... with application/octet-stream (extends ZENODO-BUCKET-PUT-415-1). Canonical: 2026-08-29 RES.032 v0.2 (POST 400 → bucket PUT, 29 files OK).
> (3) [HARD] **ZENODO-NEWVERSION-STRAY-PURGE-1** (extends ZENODO-DELETE-COUNT-VERIFY-1) — newversion drafts can carry stray files not visible in the parent record's count (canonical: 'gitignore' left from the v0.1 dotfile upload attempt); before publish: list draft files, DELETE any name not in the expected set (per-file links.self), then assert count == expected.
> (4) [SOFT] **ZENODO-CONCEPTRECID-COERCE-1** — conceptrecid arrives as int or str depending on endpoint; compare after str() coercion (canonical: 2026-08-29 RES.032 v0.2 — post-publish assert crashed on `"22159887" == 22159887` after the record was already published).
> (5) [HARD] **BUILD-PDF-BIB-FILENAME-1** (extends REFERENCE-RENDER-FROM-BIB-1) — build-pdf.py located only refs.bib; projects shipping references.bib silently skipped citeproc and shipped PDFs with literal [@key] markers (canonical: RES.032 v0.1 → user defect report; pipeline now accepts both filenames and PRINTS "CITEPROC: ... enabled/disabled" as a permanent gate). NEVER claim a PDF has expanded citations without grepping the built HTML for raw "[@" keys.
> (6) [HARD] **CITE-AUDIT-LIVE-API-1** (extends PUBLICATION-STATUS-STALE-1) — citation audits must validate sibling DOIs and titles against the LIVE Zenodo records API, not only the registry: registry rows lag (canonical: RES.031 published ~10 min before RES.032 v0.1 while its registry row still read P3/in-preparation → v0.1 shipped "in preparation, no DOI"; corrected in v0.2).
> (7) **FRONTMATTER-DUPLICATION-1 reinforced** — body H1/byline must never duplicate YAML title/author (RES.032 v0.1 shipped body H1 + byline → "duplicate title" user report; v0.2 deleted both; rendered artifacts must show exactly 1 title).
> Cross-reference: kaizen v2.114, research v2.146, cloudflare v3.69, qnfo-core v1.39, execution-mandate v2.13, session this.
...
# DEEPCHAT DEFAULT SYSTEM PROMPT v3.93
