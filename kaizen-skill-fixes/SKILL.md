...
> Existing sections A–J preserved as historical fix documentation. Added KIF-38
...
> execution skill that runs autonomously — detecting patterns, counting
...
> **v1.6:** Added J1 — KIF-31 Acronym Hallucination Gate (2026-07-26).
> Model fabricated "Zhu, Brad, Wang" as expansion of ZBW (Zitterbewegung).
> v1.5: Added I1 — KIF-30 Mandatory PDF Inclusion in Zenodo Deposits.
> v1.4: Added H1 — Cross-Domain Consilience Translation Protocol (KIF-29).
> v1.3: Added G1/G2/G3 — Epistemic Bias Fixes (KIF-16/17/18).
> v1.2: Added F1 — Buffer GraphQL API fixes.
> v1.1: Added E1 — OSF full API automation correction.
> v1.0: A1–D2 — Initial fix catalog (encoding, credentials, Windows patterns).
...
```
┌─────────────────────────────────────────────────────────────┐
│  LAYER 1: FAILURE PATTERN REGISTRY (§K)                     │
│  Canonical catalog of known failure signatures + frequency  │
│  counters. Persisted in D1 + local JSON.                    │
├─────────────────────────────────────────────────────────────┤
│  LAYER 2: SESSION SCANNER (§L)                              │
│  Auto-scans tool outputs, error messages, and conversation  │
│  text for pattern matches. Runs at session start + end.     │
├─────────────────────────────────────────────────────────────┤
│  LAYER 3: AUTONOMOUS TRIGGER LOGIC (§M)                     │
│  Severity-based frequency thresholds. When a pattern exceeds│
│  threshold → auto-execute remediation. No user prompting.   │
├─────────────────────────────────────────────────────────────┤
│  LAYER 4: REMEDIATION ENGINE (§N)                           │
│  Generates the fix (edit/write to skill files), applies it, │
│  updates KIF registry in qnfo-agent, logs to history.       │
├─────────────────────────────────────────────────────────────┤
│  LAYER 5: AUTONOMOUS VERIFICATION (§O)                      │
│  Post-fix: re-scan for same pattern, verify file integrity, │
│  check for side effects. Escalate if pattern re-emerges.    │
└─────────────────────────────────────────────────────────────┘
```
...
5. **Safe-by-default:** Infrastructure changes (D1 schema, Worker deploys, R2
   writes) still require the Production Immutability Gate (§qnfo-agent §4).
   Only skill-file edits are fully autonomous.

---

## §K: FAILURE PATTERN REGISTRY
...
| **Remediation** | Replace with write-to-file pattern: `write` _script.py → `exec python _script.py` → delete |
| **Target** | All 12 SKILL.md files — replace inline `python -c` with canonical script references |
...
| **Target** | research/cloudflare SKILL.md — 13 bare curl instances |
| **Related KIF** | KIF-05, KIF-37 |
...
| **Target** | All SKILL.md files — 12 && chains |
| **Related KIF** | KIF-05, KIF-37 |
...
| **Remediation** | Run `credential-scan.py --staged` pre-commit; remove token → env var; add to .gitignore |
...
| **Severity** | CRITICAL |
| **Symptom** | Garbled text in PDF, D1, or markdown — mojibake |
| **Remediation** | Trace to encoding root cause (PowerShell pipe, missing UTF-8 declaration). Fix source, rebuild. |
| **Target** | research/scripts/build-paper.py, qnfo-agent §8.7 |
...
| **Category** | encoding_error |
| **Severity** | CRITICAL |
| **Symptom** | Missing glyphs in rendered PDF — font lacks the character |
| **Remediation** | Run build-paper.py which preprocesses Unicode math → LaTeX math mode |
...
| **Remediation** | Run `zenodo-token-check.py` FIRST — distinguish real scope problem from transcription error |
...
| **Remediation** | Run `wrangler-check.js` — tests npx wrangler --version + whoami directly |
...
| **Category** | api_drift |
| **Severity** | HIGH |
| **Symptom** | 401/404 indistinguishable from real auth failure — wrong account/DB |
...
| **Remediation** | qnfo-agent §9.11 Rule 14: No claim without execution evidence. Scan response before delivery. |
| **Target** | qnfo-agent §9.11 |
...
| **Remediation** | qnfo-agent §7 Acronym Expansion Gate — verify against prior project artifacts |
| **Target** | qnfo-agent §7, research §5 Pre-Publication Checklist |
| **Related KIF** | KIF-31 |

#### FP-013: Edit tool Unicode matching failure
| Field | Value |
|:------|:------|
| **Signature** | `edit` tool returns "Cannot find the specified text to replace" twice on text `read` confirms exists, containing em-dashes (§) or section signs (§) |
| **Category** | tool_failure |
| **Severity** | MEDIUM |
...
| **Category** | data_loss |
| **Severity** | CRITICAL |
| **Symptom** | Content silently corrupted to "[object Object]" string — PowerShell ConvertTo-Json bug |
...
| **Target** | research §Source File Encoding Integrity |
| **Related KIF** | KIF-28 |
...
| **Category** | platform_mismatch |
| **Severity** | MEDIUM |
| **Symptom** | Silent failure or empty output — npx not found in subprocess PATH |
...
| **Category** | tool_failure |
| **Severity** | HIGH |
| **Symptom** | Silent no-op against local Miniflare simulation — writes/reads nothing durable |
| **Remediation** | Always add `--remote` to wrangler r2 commands |
| **Target** | qnfo-agent §8.6 Rule 11, cloudflare skill |
| **Related KIF** | KIF-08 |
...
| **Remediation** | Run `skill-hygiene.js` — exit 0=clean, 1=stale, 2=conflicts |
...
| **Remediation** | §8.10 QNFO-Skill Boundary Gate — detect and delete platform defaults |
| **Target** | qnfo-agent §8.10 |
| **Related KIF** | KIF-35 |
...
| **Remediation** | Elevate to skill file (memories/history.log → SKILL.md body) per 4-tier model |
| **Target** | qnfo-agent §8.9 |
| **Related KIF** | KIF-33 |
...
1. **Local cache:** `kaizen-skill-fixes/references/pattern-registry.json` —
   updated at session end, loaded at session start. Session-scoped tracking.

2. **D1 durable store:** `kaizen_patterns` table in `qnfo-audit` database —
...
4. **Check thresholds** — if any pattern is already at remediation threshold from prior sessions, apply the fix NOW before proceeding
...
|:--------|:-----------------|:-------|
| `exec` exit code ≠ 0 | Scan stderr/stdout for known error signatures | Increment pattern counter, log to session |
...
```python
# _pattern_scan.py — scan tool output for known failure signatures
...
4. **Check autonomous triggers** — if any pattern crossed threshold (§M), execute remediation NOW
5. **Log** to `memories/history.log`

---

## §M: AUTONOMOUS TRIGGER LOGIC
...
Pattern triggered
    │
    ├─ Can the fix be applied to a SKILL.md file? → AUTO-EXECUTE (§N)
    │
    ├─ Does the fix require a new canonical script? → AUTO-EXECUTE (§N)
    │
    ├─ Does the fix require D1 schema change? → FLAG [INFRA-CHANGE-NEEDED]
    │   (Production Immutability Gate — requires user)
    │
    ├─ Does the fix require Worker redeploy? → FLAG [INFRA-CHANGE-NEEDED]
    │
    └─ Pattern is new/unrecognized? → LOG to registry, categorize,
...
## §N: REMEDIATION ENGINE

### N.1 Autonomous Fix Workflow

When a pattern triggers remediation (§M), execute this workflow without user prompting:
...
2. VERIFY preconditions
   ├─ Read the target file to confirm current state
   ├─ Check KIF registry — is this already fixed?
   └─ Check for conflicting edits from other sessions

3. GENERATE fix
   ├─ For code example drift: replace with canonical script reference
   ├─ For anti-pattern: add row to Anti-Patterns table
   ├─ For gate needed: add gate to appropriate skill section
   ├─ For new script: write script to skill/scripts/
   └─ For KIF entry: add row to qnfo-agent §0.11

4. APPLY fix
   ├─ Use edit tool for surgical changes (max 2 retries, then PowerShell)
   ├─ Use write tool for new scripts
   ├─ Keep changes minimal — fix the pattern, don't restructure

5. VERIFY fix (§O)
   ├─ Re-read the modified file
   ├─ Check that the pattern wouldn't re-occur
   ├─ Run any applicable verification gate

6. LOG fix
   ├─ Add to KIF registry (new KIF-## or update existing)
   ├─ Append to memories/history.log
   ├─ Update pattern status to 'remediated'
   └─ Reset frequency counter (but keep history)
```
...
```markdown
# Pattern: FP-001/002/003 — platform-unsafe code example
...
# Fix: Add row to qnfo-agent §0.11
...
| V4 | **KIF consistency** | If a new KIF was added, verify it appears in qnfo-agent §0.11 | Row present with correct format |
...
2. If still fails → LOG [AUTONOMOUS-FIX-FAILED: <reason>]
3. Increment pattern severity by one level (LOW→MEDIUM→HIGH→CRITICAL)
4. Flag [REQUIRES-MANUAL-INTERVENTION] at session end
5. Do NOT retry more than twice — avoid infinite loop
```
...
# PART II: HISTORICAL FIX DOCUMENTATION (§A–J)

> Sections A through J below are the historical fix catalog from v1.0–v1.6.
...
> — the Autonomous Engine (§K–O above) uses them to detect recurrences of
...
**Fix:** Add Unicode→LaTeX math preprocessor to PDF build step. Now canonical in `research/scripts/build-paper.py`.

**Affected skills:** `research` §5 (PDF Building)
...
**Fix:** Now superseded by FP-001/002/003 in the Autonomous Pattern Registry (§K). Canonical scripts replace inline code. Use `;` not `&&`. Use `curl.exe` not `curl`.
...
## E: CORRECTION — OSF Registration IS Fully API-Automatable

### E1: OSF Preregistration — Full API Automation Protocol

**Prior false claim (retracted):** The research skill stated OSF registration "requires browser interaction." **This was WRONG.**

**Verified truth:** Entire OSF Preregistration — schema discovery, field population, subject taxonomy, final submission — is 100% API-achievable. Live registration `kj6ar` created via pure API, HTTP 201.

Key discoveries:
- Real schema keys look like `344-2`, `344-47` — NOT `q1`/`q2`
- Select fields require VERBATIM option text matching
- Subject taxonomy chain is MANDATORY (root→leaf)
- `pending_registration_approval: true` = email-confirm anti-hijacking, not moderation

---

## F: CRITICAL Fix — Buffer GraphQL API

### F1: Broken Inline Fragments + Missing `assets: []`

**Problems found (2026-07-22):**
1. Inline fragments on `PostActionPayload` union — retracted claim in v2.14 (fragments DO work; the bug was fragmenting on non-existent type `Post`)
2. Missing `assets: []` (NON_NULL required field)
3. Wrong `schedulingType` value (`notification` → `automatic`)
...
Created `research/scripts/buffer-post.py` — canonical CLI tool with live channel discovery.

Buffer 401 Diagnostic Protocol: NEVER diagnose "stale token" from a single 401. Run diagnostic first. Test GraphQL at `api.buffer.com` — single 401 is INSUFFICIENT EVIDENCE.
...
Agent treated "not peer reviewed" as heuristic for "fringe/unreliable." Added Institutional Status Neutrality Gate — evaluate claims against evidence, not institutional metadata. Banned "fringe"/"pseudoscience" without specific contradicting-evidence citations.

### G2: Convergence Trap (KIF-17)

Two AI systems agreeing on dismissal ≠ validation. May reflect shared training-data bias. Added AI Convergence Bias Disclosure requirement.

### G3: Symmetry Violation (KIF-18)

Documents defaulted to supporting-evidence-only structure. Added Mandatory Symmetry Template — every literature review MUST include both Supporting AND Constraining sections.

---

## H: HIGH Fix — Cross-Domain Consilience (KIF-29)

### H1: Research Stays Siloed Within a Single Domain

Added Cross-Domain Consilience Gate to research skill Phase 1. 6-domain structural translation: Physics, CS, CogSci, Information Theory, Biology, Sociology. Core Dynamic → Cross-Domain Lexicon → Domain Translations → Synthesis Consilience → Research Integration.

---

## I: CRITICAL Fix — Mandatory PDF Inclusion (KIF-30)
...
## J: CRITICAL Fix — Acronym Hallucination (KIF-31)

### J1: Model Fabricates Acronym Expansions

Model expanded "ZBW" as "Zhu, Brad, Wang" — three fabricated author initials. Correct expansion: "Zitterbewegung (ZBW)." Added Acronym Expansion Gate to qnfo-agent §7: verify every parenthetical acronym expansion against prior project artifacts. Zero prior occurrences → flag `[UNVERIFIED-ACRONYM]`.
...
| Waiting for user to report a bug before investigating | Autonomous Scanning (§L) — detect patterns from tool output automatically |
| Applying fixes only when explicitly directed | Autonomous Trigger Logic (§M) — frequency thresholds auto-trigger remediation |
| Creating KIF entries manually after user points out the bug | Remediation Engine (§N) — generates KIF entries autonomously when patterns trigger |
| Letting recurring patterns accumulate across sessions without action | D1 persistence (§K.3) — frequency counters survive session boundaries |
| Treating a single failure as a one-off | Frequency analysis — only act on RECURRING patterns (≥ threshold) |
...
- [ ] Autonomous Pattern Recognition Engine architecture documented (§K–O)
...
- [ ] Existing sections A–J preserved as historical reference
- [ ] Version bumped to 2.0.0 with frontmatter consistency
- [ ] execute_plan updated for autonomous watchdog steps
- [ ] Role change documented — from passive reference to active execution skill
