...
> **v2.151 UPDATE (2026-09-05, kaizen - CMD SKILLS UPDATE: publication-revision mandate mirror - REVISION-ALL-PUBLICATIONS-1 (user standing directive: EVERY research publication adversarially revised at least once -> >=2 versions on Zenodo; new/recent publications auto-enter the qnfo-paper-reviser audit loop; v1-only backlog drains BATCH=3 per 4h fire; settled = Zenodo version count >=2, never the living-paper version column alone) + REVISER-PUBLISH-DRAIN-1 (version_queue drafted rows -> qnfo-research-exec publishV2 Zenodo newversion + PDF/HTML regen via PDF_SVC + D1 body_md/version/doi/zenodo_doi + KG + R2 mirror per WEBSITE-SYNC-COLUMNS-1; verify new_doi + repointed columns after every publish); mirrors system-prompt v4.16 + kaizen v2.138; preserves v2.150):**
> **v2.150 UPDATE (2026-09-04, kaizen - CMD SKILLS UPDATE: publish-sync remediation mirror rows - FRONTMATTER-SYNC-PARTIAL-1 (publish half-updates body_md frontmatter: version/date bumped, doi/title stale - 7/7 recent publishes; exact-string frontmatter replace + post-publish parity probe) + D1-BODY-VERSION-LAG-1 (locale v0.5 shipped with v0.4 body_md; recovery = Zenodo deposited md) + ZENODO-DEPOSITED-MD-STALE-1 (deposited md itself stale on 22290226 + 22283879; md generator must finalize frontmatter pre-deposit) + SLUG-80-CHAR-CAP-1 (silent 80-char slug cap) + KG-NODE-ID-CONVENTIONS-1 (three paper-node id conventions; link kg_node_id at publish, node properties on current record) + FTS5-DIGIT-TOKEN-PROBE-1 (probe FTS with word tokens); mirrors system-prompt v4.10 + kaizen v2.133; preserves v2.149):**
...
> **v2.149 UPDATE (2026-09-02, research - mirror WEBSITE-SYNC-COLUMNS-1 + P7-SCORECARD-LIVE-1 + EXPERIMENT-PROGRAM-1 (v4.05/kaizen v2.128): papers.qnfo.org is DYNAMIC-via-D1 (gateway renders living-paper doi+body_md) so a version is only in sync when those columns are written - after EVERY newversion publish write record DOI to doi+zenodo_doi and replace body_md, verify live page shows new DOI and ZERO old DOI; per-paper honest traffic is 3-4 req/day baseline so experiments AGGREGATE >=2 honest signals.**
