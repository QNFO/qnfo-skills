...
async function jobBriefing(env) {
  ...
  // 6. idea proposals awaiting review (public Idea Factory queue)
  try {
    const r = await env.AUDIT.prepare("SELECT id, name, idea, status, created_at FROM idea_proposals WHERE status='new' ORDER BY created_at DESC LIMIT 8").all();
    if (r.results && r.results.length) {
      items += r.results.length;
      L.push("", "Idea Factory proposals awaiting review (" + r.results.length + "):");
      for (const pr of r.results) L.push("- id " + pr.id + " | " + String(pr.name || "anonymous").slice(0, 30) + " | " + String(pr.idea || "").slice(0, 90) + " | " + String(pr.created_at || "").slice(0, 10));
    }
  } catch (e) {}
...
async function jobWeekly(env) {
  ...
  try {
    const q = await env.AUDIT.prepare("SELECT COUNT(*) AS n FROM idea_proposals WHERE status='new'").first();
    L.push("Idea Factory proposals open: " + (q && q.n || 0));
    if (q && q.n) {
      const top = await env.AUDIT.prepare("SELECT id, name, idea FROM idea_proposals WHERE status='new' ORDER BY created_at DESC LIMIT 3").all();
      for (const pr of top.results || []) L.push("- id " + pr.id + " | " + String(pr.name || "anonymous").slice(0, 30) + " | " + String(pr.idea || "").slice(0, 90));
    }
  } catch (err) {}
