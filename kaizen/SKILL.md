...
Current: **v2.19** (kaizen — CMD SKILLS UPDATE: cloudflare v3.45 merge audit + mirror/calibration pointer sync; 2026-08-11) (kaizen — CMD EXECUTE red-team fix cycle: stale current-state pointers + §H STEP 4 cross-ref; 2026-08-11) (kaizen — D1 closeout concurrency semantics + red-team fix cycle; 2026-08-11) (kaizen — re-execution of phantom v2.13 fixes: v2.07 dedupe + wording + ordering; 2026-08-11)
...
| **TOKEN-VERIFY-SCOPE-1: Declaring a token INVALID from the wrong-scope verify endpoint (2026-08-10)** | **HARD** (mirror; owner cloudflare v3.45). /user/tokens/verify returns 1000 for ACCOUNT-scoped tokens that are fully valid for account ops. Verify at account scope (GET /accounts/{id}/d1/database, wrangler whoami). Canonical case: session bPhAUCI_FRVeZyA5Rxmsm — CF token wrongly declared dead. |
| **D1-REST-PAYLOAD-1: d1-query.py via exec fails on spaced SQL; no fallback documented (2026-08-10)** | **HARD** (mirror; owner cloudflare v3.45). skill_run is the only d1-query.py path (exec quote-mangling breaks --sql "..."); when skill_run is disabled use D1 REST `--data-binary @payload.json` + `--oauth2-bearer %CLOUDFLARE_API_TOKEN%`. Canonical case: session bPhAUCI_FRVeZyA5Rxmsm (6/6 exec d1-query failures; REST path succeeded for schema + handoff #28402 + wbs_state). |
...
docs for a future public radar endpoint. Likelihood: [HIGH] - cloudflare v3.45 + fleet script + this
mirror all carry the rule.

[CHECK: 2026-08-13] MCP Server Portals section (cloudflare v3.45) will hold: API-created portals are
...
(mcp.q08.org) documented in cloudflare v3.45.
