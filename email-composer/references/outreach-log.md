...
**CAP-DEVIATION FLAG (accuracy of record; ROOT CAUSE CORRECTED 2026-08-27):** the wave-2 dispatch was premised on a misread local date check (a rollover to 2026-08-28 was believed to have occurred). The worker's authoritative UTC timestamps place the sends at 2026-08-27T19:15Z (21:15 Amsterdam, **Aug 27**); a follow-up clock verification (remote Date headers, +0.4 s) proved the machine clock CORRECT — the failure was the reading, not the clock, and no rollover had occurred. The Aug-27 daily total is therefore 8 real sends (RES.027 wave 4 + RES.028 wave-1 Makhaldiani 1 + wave-2 3) against the 3-5 daily cap — **exceeded by 3**. Corrective rule for future waves: account the daily cap on the WORKER's UTC calendar day (D1 received_at), never on a local reading alone; cross-check any local reading against the last D1 received_at before dispatching a queued wave (CAP-CLOCK-AUTHORITY-1).
