...
> (2) [HARD] **QNFO-TEST-LITTER-NOISE-1 added (completeness audit S2, 2026-08-25)** — QNFO-domain sends with `[PREVIEW]` / "batch preview" / "outreach batch" subjects now classify NOISE inside rule 4 (was REFERENCE → re-accumulating in GTD-Reference). Prevents recurrence of the 08-25 manual purge of 8 agent test-litter items.
...
## OUTREACH-REVIEW-1 (autonomous meta-update 2026-09-01, change f595172f)

Initial outreach emails are sent fully autonomously with no user review, and only inbound replies requiring a human (meeting requests, events, collaboration proposals, decisions) are routed to the user while all others are drafted and sent on the user's behalf.

- Evidence: user directive 2026-09-01 (durable memory mem-R9CDHuREsOWg)
- Confidence: high
