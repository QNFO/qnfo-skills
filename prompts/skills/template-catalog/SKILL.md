...
```python
# Templates DEPRECATED (v2026-07-11) — use inline formats or skill references instead.
# All fill_prompt_template, get_prompt_template_parameters, list_all_prompt_template_names calls will return empty.

# Instead of templates, use:
# - skill_view("closeout-manager") for HANDOFF formats
# - read("skill SKILL.md") for protocol details
# - Inline audit formats for audit templates

# Legacy API (no longer functional):
# get_prompt_template_parameters(templateName="HANDOFF")  # returns nothing
# fill_prompt_template("HANDOFF", {...})                  # returns empty
# list_all_prompt_template_names()                        # returns []
```
